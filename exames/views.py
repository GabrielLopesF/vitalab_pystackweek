...
def solicitar_exames(request):
    ...
    if request.method == "GET":
        ...
        dia = datetime.now() + timedelta(days=3)
        
         

        return render(request, 'solicitar_exames.html', {'tipos_exames' : tipos_exames, 'solicitacao_exames': solicitacao_exames,'preco_total': preco_total, 'dia': dia,})
...
def permitir_abrir_exame(request, exame_id):
    ...
    if not exame.requer_senha:
        ...
        if exame.resultado == '':
            messages.add_message(request, constants.ERROR, 'Ainda não foi liberado o arquivo do exame')
            return redirect ('/exames/gerenciar_exames/')
